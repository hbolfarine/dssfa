...
# DSSFA function
dssfa = function(obj, length.lambda, kdssfa = NULL, print.out = F, plot.summary = F, cor.factor = F, tol.em = 1e-08){
  ...
  if(is.null(kdssfa)){
    list.bfa$k.max = obj$K
  }else{
    list.bfa$k.max = kdssfa
  }
  
  
  # Posterior sample size.
  list.bfa$M = obj$nsim/obj$thin
  ...
  # data
  list.bfa$data = obj$data
  
  # Beta post loadings. (p x k.max x M).
  list.bfa$post.loadings = obj$post.loadings
  
  # Sigma.bar from bfa.
  list.bfa$Sigma.bar.bfa = diag(apply(obj$post.sigma2,2,mean))
  
  # Posterior sample from sigma2 (M x p).
  list.bfa$Sigma.post = obj$post.sigma2
  
  # Modelo Obliquo - booleano, se modelo é obliquo
  list.bfa$cor.factor = cor.factor
  
  # Mean of the posterior loadings inner product Beta.barBeta.bar', (p x p).
  # Could be better - Tensor multiplication?
  Mat.temp = matrix(0,list.bfa$p,list.bfa$p)
  Mat = matrix(0,list.bfa$p,list.bfa$p)
  for(i in 1:list.bfa$M){
    Mat.temp = obj$post.loadings[,,i]%*%t(obj$post.loadings[,,i])
    Mat = Mat.temp + Mat
  }
  betas.prod.mean = Mat/list.bfa$M
  
  # Omega.bar = Beta.barBeta.bar'+Sigma.bar (p x p).
  list.bfa$Omega.bar = betas.prod.mean+list.bfa$Sigma.bar.bfa
  ...
  for(k.temp in 1:list.bfa$k.max){
    ...
    #                 gamma = Inf, control = list(start = "warm", min.rhozero = T, cor.factor = cor.factor ,length.rho = length.lambda))
    # 
    if(length.lambda >= 2){
      fanc.bfa = fanc(covmat = list.bfa$Omega.bar, factors = k.temp,
      ...
      fanc.bfa = fanc(covmat = list.bfa$Omega.bar, factors = k.temp, rho = 0,
                      gamma = Inf, control = list(start = "warm", cor.factor = cor.factor ,length.rho = 1, tol.em = tol.em))
    ...
    fanc.matrix.bfa = matrix(0,length.lambda,list.bfa$p*k.temp)
    for(i in 1:length.lambda){
      fanc.matrix.bfa[i,] = as.numeric(list.bfa$loadings.fanc.bfa[[i]])
    }
    list.bfa$fanc.matrix.bfa = fanc.matrix.bfa
    
    betas.bfa.fanc.full = replicate(length.lambda,matrix(0,list.bfa$p,k.temp))
    for(i in 1:length.lambda){
      betas.bfa.fanc.full[,,i] = as.matrix(list.bfa$loadings.fanc.bfa[[i]])
    }
    list.bfa$fanc.matrix.sel = betas.bfa.fanc.full # Important - works with lambada >= 1.
    ...
    if(length(mat_dif_zero) == length.lambda){
      ...
      fanc.matrix.bfa = matrix(0,length.lambda,list.bfa$p*k.temp)
      # for(i in 1:length(length.lambda)){
      fanc.matrix.bfa[i,] = as.numeric(list.bfa$loadings.fanc.bfa[[1]])
      # }
      list.bfa$fanc.matrix.bfa = fanc.matrix.bfa
      
      betas.bfa.fanc.full = replicate(1,matrix(0,list.bfa$p,k.temp))
      # for(i in 1:length(list.bfa$lambda)){
      betas.bfa.fanc.full[,,1] = as.matrix(list.bfa$loadings.fanc.bfa[[1]])
      # }
    ...
    betas.prod.bfa.fanc = replicate(length.lambda,matrix(0,list.bfa$p,list.bfa$p))
    # Oblique Product
    ...
    if(k.temp == list.bfa$k.max){
      Omega.kmax = list.bfa$betas.prod.fanc[,,length.lambda]+diag(fanc.bfa$uniquenesses[length.lambda,,1])
      list.bfa$Omega.kmax = Omega.kmax
      list.bfa = fit.fanc.post(list.bfa)
    }
    ...
    # Selecting matrices and elements with columns that are different from zero
    if(length(mat_dif_zero) == 0){
      ...
      if(length.lambda < 2){
        uniq.full[[k.temp]] = matrix(fanc.bfa$uniquenesses[-mat_dif_zero,,1],nrow = 1)
      }
